...
git add 
~~~
8. выполнить откат добавления файла в коммит (git restore --staged <file>), при этом файл переводится в состояние untracked
~~~bash
git restore --staged
~~~
...
git restore --staged .
~~~
10. выполнить коммит, для которого установить краткую заметку - описание изменений (git commit -m "заметка")
~~~bash
git commit -m ""
~~~
11. внести правки в уже сделанный коммит (параметр no-edit означает, что сообщение остается прежним, если его нужно изменить, то используется флаг -m)
~~~bash
git commit --amend --no-edit
~~~
12. «откатить» коммит (файлы в нем) к более раннему состоянию (git reset --hard <commit hash>), исключенные из коммита файлы удаляются
~~~bash
git reset --hard <commit hash>
~~~
13. посмотреть историю коммитов
~~~bash
git log
~~~
14. посмотреть историю коммитов в сокращенном виде
~~~bash
git log --oneline
~~~
---
