...
git diff 
~~~
17. использование суффикса навигации [название коммита или его хэш]~N при сравнении коммитов, где N - это число, отсчитывающее от заданного коммита (с индексом 0) заданное количество коммитов назад во времени
~~~bash
git diff HEAD~ HEAD
~~~
18. посмотреть изменения в stagged-файлах (после выполнения команды git add и до выполнения git commit)
~~~bash
git diff --staged
~~~
---
### Управление ветками:
1. просмотреть ветки проекта (команда без параметров) или создать новую (git branch [название_ветки])
~~~bash
git branch
~~~
2. отобразить все ветки
~~~bash
git branch -a
~~~
...
git checkout 
~~~
4. создать ветку и сразу переключиться на неё (git checkout -b [название_ветки])
~~~bash
git checkout -b 
~~~
5. сравнить ветки (git diff [название_ветки1] [название_ветки2])
~~~bash
git diff 
~~~
6. выполнить слияние веток (git merge [название_ветки])
~~~bash
git merge 
~~~
7. удалить ветку после объединения (git branch -D [название_ветки]). Вариант команды "git branch -d" удалит ветку только, если она была полностью объединена с другой.
~~~bash
git branch -D 
~~~
~~~bash
