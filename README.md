...
git add 
~~~
8. выполнить откат (git restore --staged <file>) откат последних изменений из состояния stage->modified, либо откат добавления файла в отслеживаемые (если он впервые был добавлен через git add)
~~~bash
git restore --staged
~~~
...
git restore --staged .
~~~
10. выполнить откат изменений в файле, который еще не был закоммичен (git restore <file>), восстанавливается последнее актуальное состояние (HEAD)
~~~bash
git restore 
~~~
